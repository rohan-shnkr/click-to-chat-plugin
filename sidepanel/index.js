...
function createMessageElement(message) {
    ...
    messageDiv.className = `thread-message message-${message.user ? 'user' : 'gemini'}`;
    
    const contentDiv = document.createElement('div');
    contentDiv.className = 'message-content';
    contentDiv.textContent = message.user || message.gemini;
...
function createThreadElement(thread) {
    ...
    headerDiv.addEventListener('click', () => {
        const isVisible = messagesDiv.style.display === 'block';
        messagesDiv.style.display = isVisible ? 'none' : 'block';
    });
